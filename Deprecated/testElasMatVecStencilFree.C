...
double** RegularGradDivStencil;
double** RegularLaplacianStencil;
...
int main(int argc, char ** argv ) {	
  ...
  RegularGradDivStencil = new doublePtr[24];
  RegularLaplacianStencil = new doublePtr[8];
  for(int i = 0; i < 24; i++) {
    RegularGradDivStencil[i] = new double[24];
  }
  for(int i = 0; i < 8; i++) {
    RegularLaplacianStencil[i] = new double[8];
  }

  InitRegularGradDivStencil(RegularGradDivStencil);
  InitRegularLaplacianStencil(RegularLaplacianStencil);
  ...
  DAMGSetKSP(damg, CreateElasticityMatStencilFree, ComputeElasticityMatStencilFree, ComputeRHS4);
  ...
  for(int i = 0; i < 24; i++) {
    delete [] RegularGradDivStencil[i];
  }
  for(int i = 0; i < 8; i++) {
    delete [] RegularLaplacianStencil[i];
  }
  delete [] RegularGradDivStencil;
  delete [] RegularLaplacianStencil;
