...
int main(int argc, char ** argv ) {	
  ...
  if(!rank) {
    std::cout<<"min coarsest level reset to: "<<minCoarsestLevel<<std::endl;
  }

  //Since we only refine the coarse octants in an already balanced octree, the
  //2:1 balance constraint will be preserved after the following operation.
  //The following loop preserves the sorted order.
  //Linearity is preserved as well.
  std::vector<ot::TreeNode> tmpBalOct;
  for(unsigned int i = 0; i < balOct.size(); i++) {
    if(balOct[i].getLevel() >= minCoarsestLevel) {
      tmpBalOct.push_back(balOct[i]);
    } else {     
      ot::TreeNode currOct(balOct[i].getX(), balOct[i].getY(),
          balOct[i].getZ(), (minCoarsestLevel-1), dim, maxDepth);
      while(true) {
        currOct.addChildren(tmpBalOct);
        unsigned char cNum = currOct.getChildNumber();       
        while((cNum == 7) && (balOct[i].isAncestor(currOct))) {
          ot::TreeNode tmpNode = currOct.getParent();
          currOct = tmpNode;
          cNum = currOct.getChildNumber();
        }
        //At this point, we need the first decendant (at level =
        //minCoarsestLevel-1) of the next brother of currOct 
        if(balOct[i].isAncestor(currOct)) {
          ot::TreeNode parOfCurr = currOct.getParent();
          std::vector<ot::TreeNode> tmpChildren;
          parOfCurr.addChildren(tmpChildren);
          currOct = ot::TreeNode(tmpChildren[cNum+1].getX(), tmpChildren[cNum+1].getY(),
              tmpChildren[cNum+1].getZ(), (minCoarsestLevel-1), dim, maxDepth);
        } else {
          break;
        }
      }
  ...
  double error = ComputeError5(DAMGGetDA(damg), DAMGGetx(damg));

  if(!rank) {
    std::cout<<"L2 Error: "<<error<<std::endl;
  }

  double testError = TestError5(DAMGGetDA(damg));
