...
int createShapeFnCoeffs(double ****& Mmat) {

#ifdef __USE_MG_INIT_TYPE3__
  createShapeFnCoeffs_Type3(Mmat);
#else
#ifdef __USE_MG_INIT_TYPE2__
  createShapeFnCoeffs_Type2(Mmat);
#else
  createShapeFnCoeffs_Type1(Mmat);
#endif
#endif
...
int createShapeFnCoeffs_Type3(double ****& Mmat) {
  FILE* infile;
  int rank;
...
int createShapeFnCoeffs_Type2(double ****& Mmat) {
  FILE* infile;
...
int createShapeFnCoeffs_Type1(double ****& Mmat) {
  FILE* infile;
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD,&rank);
  if(!rank) {
...
int destroyShapeFnCoeffs(double ****& Mmat ) {
  for(unsigned int cNum = 0; cNum < 8; cNum++) {
    for(unsigned int eType = 0; eType < 18; eType++) {
      for(unsigned int i = 0; i < 8; i++) {
