...
// The different distributions we will consider ...
...
int main(int argc, char **argv) {
  ...
  double gSize[3];
  unsigned int local_num_pts = 10000;
  unsigned int dim = 3;
  ...
  PetscLogStageRegister("Part", &stages[3]);
#endif
  ...
  if (argc < 2) {
    std::cout << "Usage: " << argv[0] << " local_num_pts dim maxDepth maxNumPts incCorner writePoints" << std::endl;
  }
  ...
  if (!rank) {
    std::cout << "Removing duplicate nodes... " << localTime << std::endl;
  }
  ...
  // Do the fake partition tests here ...
  /*
   
  1. pick a number of procs. divide up octants based on these. 
    a) without slack
    b) with slack
  2. Now, compute the boundary characteristics 
    a) number of boundary elements
    b) number of boundary faces
   
   
    Actually, compute face boundaries and then using that figure out if element is a boundary element. 
   
    Maybe its better to use ODA for this?
